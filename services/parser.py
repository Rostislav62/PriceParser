    ...
    try:
        ...
        for product in products:
            ...
            try:
                ...
                logger.error(f"Could not convert price '{price_tag.text}' for product '{name}': {e}")
                continue

            # 🆕 Ищем категорию
            class_list = product.get('class', [])
            categories = [cls.replace('product_cat-', '') for cls in class_list if cls.startswith('product_cat-')]
